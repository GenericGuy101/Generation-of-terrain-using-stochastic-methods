...
#Si queremos generar un cielo estrellado (como pareciese que se puede generar a traves de esto)
#debemos oscurecer los azules (el canal verde y azul) en particular
import numpy as np
import matplotlib.pyplot as plt
...
final_terrain = final_terrain.astype(float)

# Crear un nuevo cmap oscurecido basado en el cmap 'terrain'
terrain_cmap = plt.get_cmap('terrain')
colors = terrain_cmap(np.linspace(0, 1, 256))

# Identificar píxeles cercanos al blanco (puedes ajustar la distancia)
white_threshold = 0.3  # Ajusta este valor según tus necesidades
distances_to_white = euclidean_distances(colors[:, :3], [[1, 1, 1]])
mask = distances_to_white < white_threshold

# Crear una versión oscura de la imagen
dark_terrain = np.copy(final_terrain)

# Oscurecer solo el canal azul en los píxeles no blancos
dark_terrain[~mask, 2] *= 0.7

# Visualizar terreno generado con el nuevo cmap oscurecido
plt.figure()
plt.imshow(dark_terrain, cmap='terrain', origin='lower')
plt.colorbar()
plt.title('Cielo Nocturno Virtual (Simulated Annealing) - Oscurecido (solo azules)')
