...
def costo_terreno(terreno):
    ...
    penalizacion_pendiente = np.sum(np.abs(np.diff(terreno, axis=0))) + np.sum(np.abs(np.diff(terreno, axis=1)))

    # Calcula la recompensa de extremos
    extremos = np.array([terreno[0, 0], terreno[0, -1], terreno[-1, 0], terreno[-1, -1]])
    recompensa_extremos = np.sum(np.exp(-np.abs(np.tile(extremos, len(terreno.flatten()) // 4) - terreno.flatten())**2))



    # Calcula el costo total
    costo_total = w_homogeneidad * penalizacion_homogeneidad + w_pendiente * penalizacion_pendiente + w_extremos * recompensa_extremos
...
def simulated_annealing(initial_terrain, iterations, initial_temperature, cooling_rate):
    ...
    for iteration in range(iterations):
        ...
        new_terrain = current_terrain + generate_random_matrix(size=current_terrain.shape)
